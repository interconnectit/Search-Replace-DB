 ...
 *
 * To contribute go to
 * http://github.com/marfjeh/search-replace-db
 *
...
class icit_srdb_ui extends icit_srdb
{
    ...
    public function delete_script($path)
    {
        ...
        if (!$can_proceed) {
            return self::DELETE_SCRIPT_FAIL_UNSAFE;
        }

        foreach ($all_targets as $target_filename) {
            if (is_file($target_filename)) {
                if (false === @unlink($target_filename)) {
                    return self::DELETE_SCRIPT_FAIL_CANT_DELETE;
                }
            } else {
                if (false === @rmdir($target_filename)) {
                    return self::DELETE_SCRIPT_FAIL_CANT_DELETE;
                }
            }
        }
    ...
    public function ui()
    {
		...
		<form action="" method="post">
				...
				<div class="fields">
					...
					<div class="field radio">
						<label for="all_tables">
							<input id="all_tables" name="use_tables" value="all" type="radio" <?php if (! $this->db_valid()) {
            echo 'disabled="disabled"';
					...
					<div class="field radio">
						<label for="subset_tables">
							<input id="subset_tables" name="use_tables" value="subset" type="radio" <?php if (! $this->db_valid()) {
            echo 'disabled="disabled"';
			...
			<fieldset class="row row-results">
				...
				<div class="fields">
					...
					<span class="submit-group">
						...
						<input type="submit" name="submit[dryrun]" value="dry run" <?php if (! $this->db_valid()) {
            echo 'disabled="disabled"';
        } ?> class="db-required" />

						<input type="submit" name="submit[liverun]" value="live run" <?php if (! $this->db_valid()) {
            echo 'disabled="disabled"';
        } ?> class="db-required" />
            ...
            ?>
							<input type="submit" name="submit[innodb]" value="convert to innodb" <?php if (! $this->db_valid()) {
                echo 'disabled="disabled"';
            } ?> class="db-required secondary field-advanced" />
						<?php
        } ?>

						<input type="submit" name="submit[utf8]" value="convert to utf8 unicode" <?php if (! $this->db_valid()) {
            echo 'disabled="disabled"';
        } ?> class="db-required secondary field-advanced" />

						<input type="submit" name="submit[utf8mb4]" value="convert to utf8mb4 unicode" <?php if (! $this->db_valid()) {
            echo 'disabled="disabled"';
        } ?> class="db-required secondary field-advanced" />
    ...
    public function deleted()
    {
		...
		<section class="row row-contribute">
			...
			<div class="content">
				...
				<p>Got suggestions? Found a bug? Want to contribute code? <a href="https://github.com/marfjeh/search-replace-db">Join us on Github!</a></p>
    ...
    public function html($body)
    {
        ...
        // html classes
        $classes = array( 'no-js' );
        $classes[] = $this->regex ? 'regex-on' : 'regex-off'; ?><!DOCTYPE html>
