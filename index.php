...
class icit_srdb_ui extends icit_srdb {
	...
	public function js() {
		...
		<script>
			...
			// returns true if this text holds a PHP serialised object
			function containsSerialisedString( text )
			{
				// we can't display the dumb highlight on objects with strings (manifest as "s:digit") because this might change the length
				return ( ( /s:[\d]+:"/.exec( text ) ) ? true : false );
			}
			
			function serialisedObjectHighlightsHTML( text )
			{
				// for every occurrence of \x02\x02 \x03\x03 within a serialized string
				// replace the markers with a highlight span and decrement the s counter by four
				while ( true )
				{					
					var serialised_string_re = /s:([\d]+):"((?:[^"\\]|\\.)*?\x02\x02(?:[^"\\]|\\.)*?\x03\x03(?:[^"\\]|\\.)*?)"/;
					
					var matches = (text).match(serialised_string_re);
					
					if ( !matches ) return text;

					var digit_part = matches[1];

					digit_part -= 4;

					var string_part = matches[2];

					string_part = string_part.replace("\x02\x02", '<span class=\'highlight\'>');
					string_part = string_part.replace("\x03\x03", '</span>');

					var reassembled_string = 's:' + digit_part + ':"' + string_part + '"';
					
					text = text.substr(0, matches.index)
					     + reassembled_string
					     + text.substr(matches.index + matches[0].length);
				}
			}
									...
									$.each( changes, function( i, item ) {										
										...
										<div class="diff-wrap">\
											...
											<div class="diff">\
												...
												.find( '.to' ).text( item.to_markers ).end()
												.appendTo( $changes );
										if ( regex ) {
											...
											// Only display dumb highlights if this isn't a serialised object																						
											...
											else
											{
												...
												{
													to_div.html( serialisedObjectHighlightsHTML( markered_text ) );
												}
												else
												{
													to_div.html( item.to );
