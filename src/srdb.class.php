...
class icit_srdb
{
    ...
    public function __construct($args)
    {
        ...
        foreach ($args as $name => $value) {
            ...
            if (is_array($value) and ($name!="search" or $name!="replace")){
                $value = array_map('stripcslashes', $value);
            }
            if ((($name == "search") or ($name == "replace")) && is_array(json_decode($value,true))){
...
#                $report = $this->replacer( $this->search, $this->replace, $this->tables );
                $report = $this->replacer($this->search[0], $this->replace[0], $this->tables);
                for ($i = 1; $i < count($this->search); $i++) {
                    $report = array_merge($report, $this->replacer($this->search[$i], $this->replace[$i], $this->tables));
                }
            } else {
                $report = $this->replacer($this->search, $this->replace, $this->tables);
            }
